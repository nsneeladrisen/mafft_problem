...
if ($options=~s/ -num / /g)    {$num=1; $desclen=505;}
...
if ($options=~s/ -d\s+(\S+) / /g) {$desclen=$1;}
...
################################################################################################
	...
	$n=0;
	my $seq=<INFILE>;
	$seq=~s/^\#.*//;          # remove commentary lines at beginning of file
...
#		if ($nss>=0) {next;}  # comment out: allow for two or more >ss_dssp and >ss_pred lines
	...
	while ($line = <INFILE>)
	{
	    ...
	    if (!(exists $seqhash{$1})) 
	    {
		if ($noss && $line=~/^(aa|ss)_/) {next;} # do not read in >ss_ and >aa_ sequences
		if ($line=~/^aa_/) {next;} # do not read in >ss_ and >aa_ sequences
		if ($line=~/^ss_/) {
...
#	    print($namefieldlen.$line);
		...
		if ($line=~/^[*.: ]*$/) {next;}
		if ($noss && ($line=~/^aa_/ || $line=~/^ss_/)) {next;} # do not read in >ss_ and >aa_ sequences
		chomp($line);
...
#	    printf("%3i %3i %-16.16s %s\n",$block,$k,$names[$k],$seqs[$k]);
   ...
   # Read psi format
	...
	while ($line = <INFILE>) #scan through PsiBlast-output line by line
	{
	    ...
	    if ($line=~/^(aa|ss)_/) {next;} # do not read in >ss_ and >aa_ sequences
	    $names[$n]=$1;
	    $seqs[$n]=$2;
	    $n++;
	}
